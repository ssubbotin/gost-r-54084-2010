...
Данные извлечены из PDF методом OCR-парсинга текстового слоя. Около 98.9% значений извлечены корректно. Небольшая часть значений (~1.1%) содержит `None` из-за артефактов OCR. Рекомендуется верификация критически важных значений по оригиналу стандарта.
